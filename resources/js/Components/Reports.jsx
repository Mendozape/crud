...
import axios from 'axios'; // Added axios for report fetching
...
  // Load fees
  useEffect(() => {
    // Fetch fees including soft-deleted ones (using axios if needed for credentials/consistency)
...
      fetch(`/api/reports/search-residents?search=${encodeURIComponent(residentQuery)}`, { credentials: "include" })
        .then(res => res.json())
...
      switch (reportType) {
          case 'debtors':
              baseTitle = 'ADEUDOS POR RESIDENTE';
              break;
          case 'paymentsByResident':
...
    if (reportType === "debtors") {
        // Adjusted Headers (8 columns total for normalized addresses)
        tableHeaders.push([
            "Residente", "Dirección Completa", "Tipo de Pago", "Meses Pagados", 
            "Monto Cuota", "Meses Vencidos", "Último Pago", "Total Deuda"
        ]);
        tableBody.push(...data.map(row => [
            (row.last_name || "") + (row.name ? ", " + row.name : ""),
            row.full_address,
            row.fee_name || currentPaymentDisplayName, // fee_name now includes (Inactivo)
            row.paid_months,
...
        ]));
        tableColSpans = [7]; // 8 columns total
...
                ...(reportType === 'debtors' && {
                    2: { halign: 'left' },  // Tipo de Pago (where status is added)
                    3: { halign: 'center' }, // Meses Pagados
                    4: { halign: 'right' },  // Monto Cuota
                    5: { halign: 'center' }, // Meses Vencidos
                    6: { halign: 'center' }, // Último Pago
                    7: { halign: 'right', fontStyle: 'bold' } // Total Deuda
                }),
...
              <option value="debtors">Adeudos por residente</option>
...
              {/* === FILTER GROUP: DATE RANGE (DEBTORS/PAYMENTS BY RESIDENT) === */}
              {["debtors", "paymentsByResident"].includes(reportType) && (
                <>
                  <div className="col-md-2 col-6">
...
                        <th className="text-left">Dirección Completa</th> 
...
                    <td 
                      colSpan={reportType === "debtors" ? 7 : reportType === "paymentsByResident" ? 3 : 2} 
                      className="text-end"
                    >
                      Total:
                    </td>
