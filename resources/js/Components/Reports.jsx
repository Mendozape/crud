...
const Reports = () => {
    ...
    const [data, setData] = useState([]); // Raw data from the debtors report API
    ...
    const [currentMonthExpenses, setCurrentMonthExpenses] = useState({ expenses: [], total: 0, monthName: '', year: new Date().getFullYear() });
    ...
    const fetchCurrentMonthExpenses = async () => {
        try {
            const url = '/api/reports/current-month-expenses';
            ...
            if (json.data) {
                setCurrentMonthExpenses({
                    ...
                    total: json.data.total_amount || 0,
                    monthName: json.data.month_name || monthNames[currentMonthNum - 1],
                    year: json.data.year || currentYear,
                });
            }
        } catch (err) {
            console.error("Error fetching monthly expenses:", err);
            setCurrentMonthExpenses({ expenses: [], total: 0, monthName: monthNames[currentMonthNum - 1], year: currentYear });
        }
    };
    ...
    useEffect(() => {
        ...
        // Call fetch function for current month expenses on mount
        fetchCurrentMonthExpenses();
    }, []);

    useEffect(() => {
        // Debounce fetchReport execution
    ...
    const generatePdf = () => {
        ...
        // --- PDF LOGIC (Simplified, using original data structure for headers) ---
        ...
        const processedData = data.map(row => {
            ...
            grandTotalPaidPdf += rowTotalPaid;
        ...
        totalRow.push(formatCurrency(grandTotalPaidPdf));
                                ...
                                <table className="table table-bordered table-hover mb-0 table-striped table-sm">
                                    ...
                                    <tbody>
                                        {(() => {
                                            ...
                                            // Recalculate Grand Total Paid for the totals row based on filteredData
                                            const grandTotalPaidDisplayed = filteredData.reduce((sum, row) => {
                                                let rowTotal = 0;
                                                for (let i = 1; i <= 12; i++) {
                                                    if (!!row[`month_${i}`]) {
                                                        const amount = Number((row[`month_${i}_amount_paid`] ?? row.fee_amount) || 0);
                                                        rowTotal += amount;
                                                    }
                                                }
                                                return sum + rowTotal;
                                            ...
                                            const totalRow = (
                                                <tr key="final-totals" className="fw-bold bg-light">
                                                    <td colSpan={2} className="text-left text-primary">TOTAL INGRESOS MENSUAL:</td>
